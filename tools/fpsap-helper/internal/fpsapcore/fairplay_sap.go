...
func fairplaySAPHash(block []byte) (out [16]byte) {
	...
	// The first 155 steps have irregular indices, because i is unsigned and the
	// subtraction wraps through 2^32 before the modulo; they come from the tables
	// in ring.go. From i=155 the four indices are just counters at (0, 98, 142,
	...
	// hash covers work[:20] and matrix work[:35], so the three ranges are fixed
	// and the per-step range tests are not needed.
	for i := 0; i < len(state.hash); i++ {
		out[i&15] ^= work[i] ^ state.matrix[i] ^ state.hash[i]
	}
	for i := len(state.hash); i < len(state.matrix); i++ {
		out[i&15] ^= work[i] ^ state.matrix[i]
	}
	for i := len(state.matrix); i < len(work); i++ {
		out[i&15] ^= work[i]
	}

	// Reverse scramble, as one linear map -- see scramble.go.
	applyScramble(&out)
	return
}
